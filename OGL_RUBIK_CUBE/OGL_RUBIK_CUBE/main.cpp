...
int main()
{
    ...
    unsigned char* data = stbi_load("C:\\gl\\resources\\textures\\cuboTexture.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
   ...
   ourShader.setInt("texture2", 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.5f, -0.5f, 0.0f));
        ...
        if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS)
        {
            ...
            if (aumento == 360)
                aumento = 0;
            model = glm::rotate(model, glm::radians(aumento), glm::vec3(0.5f, -0.5f, 0.0f));
            model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));
        }
